...
var msgHdrUtils = require('./msgHdrUtils')
...
function FakeEditor (aIframe) {
  this.iframe = aIframe;
  this.editor = compose.getEditorForIframe(aIframe);
...
function sendMessage(aParams,
  ...
  switch (compType) {
    ...
    case mCompType.Draft: {
      ...
      // about to send...
      let msgHdr = msgHdrUtils.msgUriToMsgHdr(urls[0]);
      references =  [ ...misc.range(0, msgHdr.numReferences) ].map(i => msgHdr.getStringReference(i));
      break;
    }
    ...
    case mCompType.ForwardAsAttachment: {
      for (let url of urls) {
        let msgHdr = msgHdrUtils.msgUriToMsgHdr(url);
        references.push(msgHdr.messageId);
      }
      break;
    }
  ...
  if (popOut) {
    let composeHtml = compose.determineComposeHtml(identity);
    ...
    aBody.match({
      plainText: function (body) {
        if (composeHtml)
          fields.body = compose.plainTextToHtml(body);
        else
          fields.body = body;
      },
      editor: function (iframe) {
        ...
        if (composeHtml)
          ...
          fields.body = compose.htmlToPlainText(html);
      },
    });
    ...
    aBody.match({
      plainText: function(body) {
        ...
        fields.body = compose.simpleWrap(body.replace(/ +$/gm, ""), 72)+"\n";
      ...
      editor: function (iframe) {
        ...
        try {
          ...
          gMsgCompose.editor = compose.getEditorForIframe(iframe);
        }
